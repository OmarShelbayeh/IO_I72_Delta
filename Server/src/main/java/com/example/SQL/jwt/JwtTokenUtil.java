...
import com.example.SQL.service.UserDetailsImpl;
...
public class JwtTokenUtil implements Serializable {
  ...
  @Value("${jwt.token.expiration.remember.in.seconds}")
  ...
  public String refreshToken(String token) {
    final Date createdDate = clock.now();
    final Date expirationDate = calculateExpirationDate(createdDate, true);
  ...
  private Date calculateExpirationDate(Date createdDate, boolean rememberMe) {
    if(!rememberMe){
      ...
      return new Date(createdDate.getTime() + expirationRemember * 1000);
    }
  }
